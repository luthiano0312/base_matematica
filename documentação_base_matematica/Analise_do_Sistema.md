...
| RN17 | No modo "Progressão" do filtro de dificuldade, as questões retornadas são divididas em terços por dificuldade (1/3 fácil, 1/3 médio, 1/3 difícil). O campo "Quantidade de questões" (ver extensão de RF06 abaixo) é obrigatório nesse modo, usado para calcular a divisão. ⚠️ Regra de arredondamento quando a quantidade não for divisível por 3 ainda não definida → ver [[Pendencias#RN17]] | ^rn17
...
| RF12 | Sistema deve priorizar/filtrar questões e conteúdo recomendado com base nos interesses marcados. **Comportamento do filtro vazio (atualizado):** quando nenhum filtro é aplicado, o sistema segue um fallback em cascata: (1) conteúdo marcado como interesse no onboarding (`user_interests`); (2) se não houver interesse marcado (RN13), cai para o conteúdo com mais acertos (desempenho); (3) se nenhum dos dois existir (aluno novo, sem interesse e sem histórico), o comportamento ainda está em aberto → ver [[Pendencias#Fallback-aluno-novo]]. ⚠️ O critério de escolha quando o aluno marca mais de um interesse também ainda não foi definido → ver [[Pendencias#Multiplos-interesses]]. | ^rf12
...
| RNF08 | A senha do aluno deve seguir requisitos mínimos de segurança. ⚠️ Requisitos exatos (tamanho mínimo, uso de números/maiúsculas, confirmação de senha) ainda não definidos → ver [[Pendencias#RNF-senha]] | ^rnf08
...
| difficulty (nível de dificuldade) | enum | fácil / intermediário / difícil |
